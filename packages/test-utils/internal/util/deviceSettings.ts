...
export const setGlobalDeviceSettings = (
  settings: Partial<typeof GLOBAL_DEVICE_SETTINGS>,
  options?: SetGlobalDeviceSettingsOptions
) => {
  if (options?.resetOtherFields) {
    resetGlobalDeviceSettings()
  }

  for (const [key, value] of Object.entries(settings)) {
    if (key in GLOBAL_DEVICE_SETTINGS) {
      if (typeof value === "number") {
        GLOBAL_DEVICE_SETTINGS[key as keyof DeviceSettings] = Math.floor(value)
      } else {
        throw new TypeError(
          `Expected ${key} to be a number, got '${typeof value}' (value: ${value})`
        )
      }
    }
  }
  if (
    GLOBAL_DEVICE_SETTINGS.minSampleRate > GLOBAL_DEVICE_SETTINGS.maxSampleRate
  ) {
    throw new RangeError(
